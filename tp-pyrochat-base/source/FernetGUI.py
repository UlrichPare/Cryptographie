...
class FernetGUI(CipheredGUI):
    ...
    def encrypt(self, message: str) -> bytes:
        encrypted_data = self._fernet.encrypt(message.encode())
        self._log.debug(f"Message est chiffré")
    ...
    def decrypt(self, encrypted_data: bytes) -> str:
        try:
            ...
            self._log.error(f"Erreur de déchifrement:{str(e)}")
            return "<déchifrement échoué>"
